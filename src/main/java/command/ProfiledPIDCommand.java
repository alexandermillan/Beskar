...
/**
 * A command that controls an output with a {@link ProfiledPIDController}. Runs
 * forever by default - to add exit conditions and/or other behavior, subclass
 * this class. The controller calculation and output are performed synchronously
...
public class ProfiledPIDCommand extends CommandBase {
	 ...
	 * ProfiledPIDController. Goal velocity is specified.
	 *
	 * @param controller
	...
	public ProfiledPIDCommand(ProfiledPIDController controller, DoubleSupplier measurementSource,
			Supplier<State> goalSource, BiConsumer<Double, State> useOutput, Subsystem... requirements) {
	...
	/**
	 * Creates a new PIDCommand, which controls the given output with a
	 * ProfiledPIDController. Goal velocity is implicitly zero.
	 *
	 * @param controller
	 *            the controller that controls the output.
	 * @param measurementSource
	 ...
	 */
	public ProfiledPIDCommand(ProfiledPIDController controller, DoubleSupplier measurementSource,
			DoubleSupplier goalSource, BiConsumer<Double, State> useOutput, Subsystem... requirements) {
		requireNonNullParam(controller, "controller", "SynchronousPIDCommand");
		requireNonNullParam(measurementSource, "measurementSource", "SynchronousPIDCommand");
		requireNonNullParam(goalSource, "goalSource", "SynchronousPIDCommand");
		requireNonNullParam(useOutput, "useOutput", "SynchronousPIDCommand");
	 ...
	 *            the subsystems required by this command
	 */
	public ProfiledPIDCommand(ProfiledPIDController controller, DoubleSupplier measurementSource, State goal,
			BiConsumer<Double, State> useOutput, Subsystem... requirements) {
	 ...
	 *            the subsystems required by this command
	 */
	public ProfiledPIDCommand(ProfiledPIDController controller, DoubleSupplier measurementSource, double goal,
			BiConsumer<Double, State> useOutput, Subsystem... requirements) {
		this(controller, measurementSource, () -> goal, useOutput, requirements);
	}
