...
public class MecanumControllerCommand extends CommandBase {
	 ...
	 * @param maxWheelVelocityMetersPerSecond
	 *            The maximum velocity of a drivetrain wheel.
	 * @param frontLeftController
	 *            The front left wheel velocity PID.
	 * @param rearLeftController
	 *            The rear left wheel velocity PID.
	 * @param frontRightController
	 *            The front right wheel velocity PID.
	 * @param rearRightController
	 *            The rear right wheel velocity PID.
	 * @param currentWheelSpeeds
	 *            A MecanumDriveWheelSpeeds object containing the current wheel
	 *            speeds.
	 * @param outputDriveVoltages
	...
	public MecanumControllerCommand(Trajectory trajectory, Supplier<Pose2d> pose, SimpleMotorFeedforward feedforward,
			...
			Consumer<MecanumDriveMotorVoltages> outputDriveVoltages, Subsystem... requirements) {
		...
		m_xController = requireNonNullParam(xController, "xController", "MecanumControllerCommand");
		m_yController = requireNonNullParam(yController, "yController", "MecanumControllerCommand");
		m_thetaController = requireNonNullParam(thetaController, "thetaController", "MecanumControllerCommand");
		...
		m_frontLeftController = requireNonNullParam(frontLeftController, "frontLeftController",
				"MecanumControllerCommand");
		m_rearLeftController = requireNonNullParam(rearLeftController, "rearLeftController",
				"MecanumControllerCommand");
		m_frontRightController = requireNonNullParam(frontRightController, "frontRightController",
				"MecanumControllerCommand");
		m_rearRightController = requireNonNullParam(rearRightController, "rearRightController",
				"MecanumControllerCommand");

		m_currentWheelSpeeds = requireNonNullParam(currentWheelSpeeds, "currentWheelSpeeds",
				"MecanumControllerCommand");

		m_outputDriveVoltages = requireNonNullParam(outputDriveVoltages, "outputDriveVoltages",
				"MecanumControllerCommand");
	...
	/**
	 * Constructs a new MecanumControllerCommand that when executed will follow the
	 * provided trajectory. The user should implement a velocity PID on the desired
	 * output wheel velocities.
	 *
	 * <p>
	 * Note: The controllers will *not* set the outputVolts to zero upon completion
	 * of the path - this is left to the user, since it is not appropriate for paths
	 * with non-stationary end-states.
	 *
	 * <p>
	 * Note2: The rotation controller will calculate the rotation based on the final
	 * pose in the trajectory, not the poses at each time step.
	 *
	 * @param trajectory
	 *            The trajectory to follow.
	 * @param pose
	 ...
	 * @param maxWheelVelocityMetersPerSecond
	 *            The maximum velocity of a drivetrain wheel.
	 * @param outputWheelSpeeds
	 *            A MecanumDriveWheelSpeeds object containing the output wheel
	 *            speeds.
	 * @param requirements
	 *            The subsystems to require.
	 */
	...
	@SuppressWarnings({"PMD.ExcessiveParameterList", "ParameterName"})
	public MecanumControllerCommand(Trajectory trajectory, Supplier<Pose2d> pose, MecanumDriveKinematics kinematics,
			PIDController xController, PIDController yController, ProfiledPIDController thetaController,

			double maxWheelVelocityMetersPerSecond,

			Consumer<MecanumDriveWheelSpeeds> outputWheelSpeeds, Subsystem... requirements) {
		m_trajectory = requireNonNullParam(trajectory, "trajectory", "MecanumControllerCommand");
		m_pose = requireNonNullParam(pose, "pose", "MecanumControllerCommand");
		...
		m_xController = requireNonNullParam(xController, "xController", "MecanumControllerCommand");
		m_yController = requireNonNullParam(yController, "xController", "MecanumControllerCommand");
		m_thetaController = requireNonNullParam(thetaController, "thetaController", "MecanumControllerCommand");
		...
		m_outputWheelSpeeds = requireNonNullParam(outputWheelSpeeds, "outputWheelSpeeds", "MecanumControllerCommand");
	...
	public void execute() {
		...
		if (m_usePID) {
			...
			frontLeftOutput = frontLeftFeedforward + m_frontLeftController
					.calculate(m_currentWheelSpeeds.get().frontLeftMetersPerSecond, frontLeftSpeedSetpoint);

			rearLeftOutput = rearLeftFeedforward + m_rearLeftController
					.calculate(m_currentWheelSpeeds.get().rearLeftMetersPerSecond, rearLeftSpeedSetpoint);

			frontRightOutput = frontRightFeedforward + m_frontRightController
					.calculate(m_currentWheelSpeeds.get().frontRightMetersPerSecond, frontRightSpeedSetpoint);

			rearRightOutput = rearRightFeedforward + m_rearRightController
					.calculate(m_currentWheelSpeeds.get().rearRightMetersPerSecond, rearRightSpeedSetpoint);

			m_outputDriveVoltages.accept(
					new MecanumDriveMotorVoltages(frontLeftOutput, frontRightOutput, rearLeftOutput, rearRightOutput));

		} else {
			m_outputWheelSpeeds.accept(new MecanumDriveWheelSpeeds(frontLeftSpeedSetpoint, frontRightSpeedSetpoint,
					rearLeftSpeedSetpoint, rearRightSpeedSetpoint));
		}
